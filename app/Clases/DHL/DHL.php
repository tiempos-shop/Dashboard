...
class DHL extends API
{
    ...
    private function RequestedShipmentInternacional(array $SpecialServices,array $Ship,array $Packages,string $ShipTimestamp, $subtotal):array{
        return
            [
                "DropOffType" => "REGULAR_PICKUP",
                "ServiceType" => "P",
                ...
                "ShipTimestamp" => $ShipTimestamp,
                "UnitOfMeasurement" => $this->UnitOfMeasurement,
                "DeclaredValue" =>$subtotal, //"0000000200"
                "DeclaredValueCurrecyCode"=> "MXN",
                "PaymentInfo" => $this->PaymentInfo,
                "Account" => $this->Account
            ];
    }

    private function SpecialServices($monto_seguro,string $CurrencyCode, $isShip = false):array{
    ...
    public function GetRateRequest($precio,$currency,int $shipper_cp,string $receiver_cp,int $products_number,$weight,$length,$width,$height, $codigoPais):array{
        $special_services=$this->SpecialServices($precio,"MXN");

        $shipper=new ShipType($shipper_cp, "MX");
        $receiver=new ShipType($receiver_cp, $codigoPais);
        $ship=$this->Ship($shipper,$receiver);
        $packages=$this->Packages($products_number,$weight,$length,$width,$height);
  
        $date=date('Y-m-d');
        //DHL = > 2021-07-27T12:00:00GMT-06:00
        $time_stamp=$date."T12:00:00GMT-06:00";
    ...
    public function GetRateRequestInternacional($precio,$currency,int $shipper_cp,int $receiver_cp,int $products_number,$weight,$length,$width,$height, $codigoPais):array{
        $special_services=$this->SpecialServices($precio,"USD");
        ...
        $request_shipment=$this->RequestedShipmentInternacional($special_services,$ship,$packages,$time_stamp, $precio);
        return $this->RateRequest($request_shipment);
    }

    public function RateApiCall($precio,string $currency,int $shipper_cp,string $receiver_cp,int $products_number,$weight,$length,$width,$height, $codigoPais):array{
        $request=$this->GetRateRequest($precio,$currency,$shipper_cp,$receiver_cp,$products_number,$weight,$length,$width,$height, $codigoPais);
        return $this->CALL_DHL($this->RateUrl,$request);
    }
    public function RateApiCallInternacional($precio,string $currency,int $shipper_cp,string $receiver_cp,int $products_number,$weight,$length,$width,$height, $codigoPais):array{

        $request=$this->GetRateRequestInternacional($precio,$currency,$shipper_cp,$receiver_cp,$products_number,$weight,$length,$width,$height, $codigoPais);
    ...
    public function ShipmentRequested($ShipmentInfo, $PostalCode,$PersonName, $CompanyName, $PhoneNumber,$EmailAddress, $StreetLines, $City , $CountryCode, PackageModel $packageModel):array{
        ...
        $receiver = new ShipType($PostalCode, "MX");
    ...
    public function ShipmentInfoInternacional($precio,$currency){
        return
            [
                "DropOffType" => "REGULAR_PICKUP",
                "ServiceType" => "P",
